...
bool_e checkForBeat(float value);
void decreaseThreshold();

static BeatDetectorState state = BEATDETECTOR_STATE_INIT;
static float threshold = BEATDETECTOR_MIN_THRESHOLD;
static float beatPeriod = 0;
static float lastMaxValue = 0;
static uint32_t tsLastBeat = 0;



bool_e BEATDETECTOR_addSample(float sample)
{
...
bool_e checkForBeat(float sample)
{
    bool_e beatDetected = FALSE;
